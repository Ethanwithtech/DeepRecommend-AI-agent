...
class MusicRecommender:
    ...
    def get_recommendations(self, user_id, top_n=10):
        ...
        Returns:
            推荐歌曲列表
        """
        # 如果混合推荐系统已初始化，使用它获取推荐
        if self.hybrid_recommender is not None:
            try:
                return self.hybrid_recommender.recommend(user_id, top_n=top_n)
            except Exception as e:
                logger.error(f"使用混合推荐器失败: {str(e)}")
                # 失败时回退到简单推荐
        ...
        # 检查用户是否有评分历史
        if user_id in self.user_ratings and len(self.user_ratings[user_id]) > 0:
            # 用户已有评分历史，使用协同过滤推荐
            recommendations = self._get_cf_recommendations(user_id, top_n)
        else:
            # 用户没有评分历史，推荐热门歌曲
            recommendations = self.get_popular_songs(top_n)
            
        return recommendations
    
    def recommend(self, user_id, top_n=10, context=None):
        """
        获取个性化推荐，支持上下文感知
        ...
        if context and 'emotion' in context:
            ...
            # 使用情绪推荐
            return self.get_recommendations_by_emotion(emotion, top_n)
        
        # 默认使用普通推荐
        return self.get_recommendations(user_id, top_n)
    
    def get_songs_by_genre(self, genre, limit=10):
        """
        按音乐风格获取歌曲
        ...
        try:
            ...
            matches = self.songs_df[self.songs_df['genre'].str.lower() == genre_lower]
            
            # 如果没有精确匹配，尝试部分匹配
            if len(matches) < limit:
                matches = self.songs_df[self.songs_df['genre'].str.lower().str.contains(genre_lower)]
            
            # 如果仍然没有足够的匹配，返回随机歌曲
            if len(matches) < limit:
                remaining = limit - len(matches)
                random_songs = self.songs_df.sample(n=min(remaining, len(self.songs_df)))
                matches = pd.concat([matches, random_songs]).drop_duplicates()
            ...
            for _, song in matches.iterrows():
                result.append({
                    ...
                    'genre': song['genre'],
                    'year': int(song['year']) if 'year' in song and pd.notna(song['year']) else None
                })
            
            return result
            
        except Exception as e:
            logger.error(f"按风格获取歌曲失败: {str(e)}")
            # 返回空列表作为备用
            return []
